...
class TestWebBookingApp(unittest.TestCase):
    ...
    def test_delete_booking_single_mode_keeps_other_group_items(self):
        status, body = self.request(
            "POST",
            "/api/bookings",
            {
        ...
        status, _ = self.request(
            "DELETE",
            "/api/bookings",
            {"admin_password": "admin123", "booking_id": booking_id, "delete_mode": "single"},
        )
        self.assertEqual(status, 200)

        status, body = self.request("GET", "/api/bookings?date=2026-04-08")
        self.assertEqual(status, 200)
